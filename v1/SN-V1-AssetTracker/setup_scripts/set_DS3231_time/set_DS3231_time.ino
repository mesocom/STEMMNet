 ...
 *  - Set the clk of the DS3231
 *  - Echo the value from the DS3231 during 5 seconds
 *****************************************************************************************************/
void loop() {
  // If something is coming in on the serial line, it's
  // a time correction so set the clk accordingly.
  if (Serial.available()) {
    inputDateFromSerial();

    clk.setClockMode(false);  // set to 24h
    ...
    clk.setDoW(dow);
    clk.setHour(hour);
    clk.setMinute(minute);
    clk.setSecond(second);
    
    // Give time at next five seconds
    for (uint8_t i = 0; i < 5; i++){
        delay(1000);
        Serial.print(clk.getYear(), DEC);
        Serial.print("-");
        Serial.print(clk.getMonth(century), DEC);
        Serial.print("-");
        Serial.print(clk.getDate(), DEC);
        Serial.print(" ");
        Serial.print(clk.getHour(h12Flag, pmFlag), DEC); //24-hr
        Serial.print(":");
        Serial.print(clk.getMinute(), DEC);
        Serial.print(":");
        Serial.println(clk.getSecond(), DEC);
    }
